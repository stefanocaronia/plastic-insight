...
The initial refresh now uses one filtered root status instead of enumerating the project tree or launching one process per file, so Rider's Changes view can receive the complete controlled-change set immediately after mapping. Ordinary small dirty sets retain exact-file queries; larger or recursive sets are coalesced. A manual mapped-workspace test on 2026-08-19 confirmed the native modified-line gutter, a populated controlled-changes list, and Rider's byte-backed side-by-side diff. Private/unversioned reporting and save/external-state refresh validation remain explicit Phase 3 gates. The `ChangeProvider` reports no state for unsupported cases, while baseline retrieval errors remain explicit and never return synthetic content bytes.
