...
As of 2026-08-19, 129 automatic Kotlin/JUnit tests pass and one read-only live gateway test is skipped unless explicitly enabled; that opt-in test also passes separately against a representative Plastic file. `test`, `buildPlugin`, `verifyPluginProjectConfiguration`, and `verifyPluginStructure` pass against the local Rider 2026.2 SDK and JetBrains Runtime 25. Pure tests do not depend on Rider's bundled test framework; that dependency will be introduced only when IntelliJ fixture tests require it. The current installable artifact is `build/distributions/plastic-insight-0.1.8-SNAPSHOT.zip`. Project verification emits the known recommendation to remove the deliberate `262.*` upper compatibility bound; compatibility policy remains a Phase 5 decision.
